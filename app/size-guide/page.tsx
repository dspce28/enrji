...
import { TEE_SIZES, SWEAT_SIZES } from '@/lib/sizes';
  ...
  return (
    <div className="container">
      ...
      <div className="prose">
        ...
        <table className="size-table" style={{ fontSize: 16 }}>
          <thead><tr><th>Size</th><th>Chest (in)</th><th>Length (in)</th></tr></thead>
          <tbody>{SWEAT_SIZES.map((s) => <tr key={s.size}><td>{s.size}</td><td>{s.chest}</td><td>{s.length}</td></tr>)}</tbody>
        </table>
        <p style={{ fontSize: 15, color: 'var(--muted)', marginTop: 12 }}>250 GSM combed cotton, unisex regular fit. 3XL measurements aren&apos;t published yet — <Link href="/contact" style={{ textDecoration: 'underline' }}>message us</Link> before ordering it. The Live Like Krishna edition has a relaxed fit, so go one size up if you&apos;re between sizes.</p>
      </div>
    </div>
  );
}
